...
#ifdef CONFIG_POWERSUSPEND
static void stk3x1x_power_suspend(struct power_suspend *handler)
{
	struct stk3x1x_data *ps_data = container_of(handler, struct stk3x1x_data, stk_power_suspend);
#ifndef STK_POLL_PS
	int err;
#endif
...
static void stk3x1x_late_resume(struct power_suspend *handler)
{
	struct stk3x1x_data *ps_data = container_of(handler, struct stk3x1x_data, stk_power_suspend);
#ifndef STK_POLL_PS
	int err;
                        ...
                        const struct i2c_device_id *id)
{
	...
	ps_data->stk_power_suspend.resume = stk3x1x_late_resume;
